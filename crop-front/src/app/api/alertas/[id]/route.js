  ...
  try {
    ...
    if (sensores.length === 0) {
      return NextResponse.json({ 
        message: 'Nenhum sensor encontrado para verificação', 
        novos_alertas: 0,
        alertas: [] 
      });
    }
    ...
    for (const dado of dadosSensores) {
      ...
      const { data: alertaExistente, error: alertaError } = await supabase
        ...
        .eq('sensor_id', sensor.id)
        .eq('verificado', false)
        .gt('criado_em', trintaMinutosAtras)
        .limit(1);
      
      if (alertaError) throw alertaError;
      
      // Se já existe um alerta recente não verificado, pular para o próximo sensor
      ...
      switch (sensor.tipo) {
        ...
        default:
          continue; // Pular sensores de tipos não suportados
      }
